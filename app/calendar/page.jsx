...
export default function CalendarPage() {
  ...
  useEffect(() => {
    ...
    // This would also fetch customers
  ...
  const handleEventDrop = async (eventId, newDate) => {
    try {
      ...
      if (event.measureDate) {
        ...
        updatedEvent.date = newMeasureDate
        updatedEvent.type = 'measure'
      } else if (event.installDate) {
        const newInstallDate = addDays(
          new Date(event.installDate),
          daysDifference
        )
        updatedEvent.date = newInstallDate
        updatedEvent.type = 'install'
      }

      // Update the event
      await updateScheduleEvent(eventId, updatedEvent)
                        ...
                        <div
                          ...
                          className={`${
                            ...
                            view === 'week'
                              ? 'h-full min-h-[150px]'
                              : 'aspect-square'
                          } overflow-hidden flex flex-col transition-all duration-300 hover:bg-blue-400`}
                        >
                          <DroppableDay
                            date={day}
